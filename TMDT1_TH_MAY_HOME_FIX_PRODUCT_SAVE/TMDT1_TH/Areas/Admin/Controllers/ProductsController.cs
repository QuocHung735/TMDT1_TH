...
public class ProductsController : Controller
{
    ...
    public async Task<IActionResult> Save(ProductEditorViewModel model, string? mode)
    {
        ...
        if (!ModelState.IsValid)
        {
            ModelState.AddModelError(
                string.Empty,
                "Sản phẩm chưa được lưu. Vui lòng kiểm tra các trường được đánh dấu lỗi bên dưới.");
    ...
    private async Task ValidateReferencesAsync(ProductEditorViewModel model)
    {
        ...
        if (!marketExists)
        {
            ...
            ModelState.AddModelError(
                string.Empty,
                "Hãy tạo hoặc bật ít nhất một thị trường tại Quản lý thị trường trước khi lưu sản phẩm.");
        }
    }
