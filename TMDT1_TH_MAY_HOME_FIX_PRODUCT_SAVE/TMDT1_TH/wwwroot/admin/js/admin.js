(() => {
    ...
    const unlimitedPrice = document.querySelector('[data-unlimited-price]');
    const priceEndField = document.querySelector('[data-price-end-field]');
    const refreshUnlimitedPrice = () => {
        if (!unlimitedPrice || !priceEndField) return;
        const endInput = priceEndField.querySelector('input');
        priceEndField.classList.toggle('is-disabled', unlimitedPrice.checked);
        if (endInput) {
            endInput.disabled = unlimitedPrice.checked;
            if (unlimitedPrice.checked) endInput.value = '';
        }
    };
    unlimitedPrice?.addEventListener('change', refreshUnlimitedPrice);
    refreshUnlimitedPrice();
    ...
    const generateVariants = () => {
        ...
        variantBody.innerHTML = variants.map((variant, index) => `
            <tr>
                ...
                <td><input class="table-input" value="HOME-${slug(variant.color)}-${variant.size.toUpperCase()}" /></td>
